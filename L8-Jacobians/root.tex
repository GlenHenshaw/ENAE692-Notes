...
\begin{displaymath}
	_{2}^{0}R(\underline{q}) = \left[\begin{array}{ccc} c_{\theta} & s_{\theta} & 0 \\ -s_{\theta} & c_{\theta} & 0 \\ 0 & 0 & 1 \end{array}\right]
\end{displaymath}
so
\begin{displaymath}
...
and
\begin{eqnarray}
R\sum_{j=1}^{N}\frac{\partial R(\underline{q})}{\partial q_{j}} = \left[ \begin{array}{ccc}  0 & -1 & 0 \\ 1 & 0 & 0 \\ 0 & 0 & 0 \end{array}\right]
\end{eqnarray}
...
\begin{displaymath}
	\omega_{z} = (s_{\theta}^{2}+c_{\theta}^{2})\dot{\theta} = \dot{\theta}
\end{displaymath}
so
\begin{displaymath}
J_{\text{rot}} = \left[ \begin{array}{cc} 0 & 1 \end{array} \right]
\end{displaymath}
and the entire Jacobian is generally written as 
\begin{displaymath}
J = \left[ \begin{array}{cc} c_{\theta} & -r\ s_{\theta} \\ s_{\theta} & -r\ c_{\theta} \\ 0 & 1 \end{array} \right]
\end{displaymath}
...
The formal proof of this is something we ask you to examine in Exercise 5.17.
...
\end{displaymath}
Assembling these vectors gives:
\begin{displaymath}
^{2}J_{trans} = \left[ \begin{array}{ccc} l_{1}s_{2} - l_{3}s_{3} & -l_{3}s_{3} & -l_{3}s_{3} \\ l_{1}c_{2}+l_{2} + l_{3}c_{3} & l_{2}+l_{3}c_{3} & l_{3}c_{3} \\ 0 & 0 & 0 \end{array} \right]
\end{displaymath}
So the cross product method yields the same results for $^{2}J_{trans}$. Notice how much easier it is to compute using this method!
\end{document}
