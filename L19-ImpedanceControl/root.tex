...
\end{split}
\end{equation}
Note that the first term of Equation \ref{stiffness} is what we derived for the Salisbury stiffness controller. The second term accounts for the change of the Jacobian as the joints move; I suspect, but I have not actually investigated this, that if you are recalculating the Jacobian every time you execute your compliance control loop, this term is unnecessary.
...
\draw[-stealth] (sum2.east) -- (M.west) node[midway,above](x){$\underline{\tau}_{c}$};
\draw[-stealth] (M.east) -- (sum3.west) node[midway,above](x){$\underline{\tau}$};
\draw[-stealth] (sum3.east) -- (robot.west) node[midway,above](x){};

\draw[-stealth] (robot.east) -- (J.west) node[midway](theta){} node[midway,above](theta2){$\underline{\theta}, \underline{\dot{\theta}}$};
...
%(\XCoord, \YCoord-19) node[pos=0.15,above]{$\underline{x}$} -- 
%X feedback line
\draw[-stealth] (output.center) -- ++ (0,-4.5) -- (\XCoord, -4.5) -- (sum.south) node[below,left]{$-$};

% theta feedback line
\ExtractCoordinate{sum5.south}
\draw[-stealth] (theta.center) -- ++ (0,-4.0) -- (\XCoord, -4.0) -- (sum5.south) node[below,left]{$-$};
\ExtractCoordinate{sum4.south}
\draw[-stealth] (theta.center) -- ++ (0,-3.75) -- (\XCoord, -3.75) -- (sum4.south) node[below,left]{$-$};
\ExtractCoordinate{model.east}
\draw[-stealth] (theta.center) -- ++ (0,\YCoord) -- (model.east);

\ExtractCoordinate{sum3.south}
\draw[-stealth] (model.west) -- (\XCoord, \YCoord-47) node[pos=0.25,above]{} -- (sum3.south) node[below,left]{$+$};

\ExtractCoordinate{output.north}
\draw[<-] (sum3.north) -- ++ (0, 0.5) -- (\XCoord+15, \YCoord+20) node[above] () {$\underline{\tau}_{e}$};

\draw[-stealth] (Kp.north) -- (sum2.south) node[below,left]{$+$};
\draw[-stealth] (Kd.north) -- (sum2.south) node[below,left]{$+$};
\draw[-stealth] (Kd.north) -- (sum2.south) node[below,left]{$+$};
\draw[-stealth] (sum4.north) -- (Kd.south);
\draw[-stealth] (sum5.north) -- (Kp.south);
\ExtractCoordinate{sum4.west}
\draw[-stealth] (-1.5, \YCoord) node[left]{$\underline{\dot{\theta}}_{d}$} -- (sum4.west) node[above]{$+$};
\ExtractCoordinate{sum5.west}
\draw[-stealth] (-1.5, \YCoord) node[left]{$\underline{\theta}_{d}$}-- (sum5.west) node[above]{$+$};
\end{tikzpicture}
\end{figure}
...
\draw[-stealth] (sum.east) -- (Zd.west) node[midway,above](x){};
\draw[-stealth] (Zd.east) -- (sum2.west) node[midway,above](x){};

%\draw[-stealth] (Zd.east) -- (Jinv.west) node[midway,above](x){$\underline{F}_{d}$};
%\draw[-stealth] (Jinv.east) -- (sum2.west) node[midway,above](x){$\underline{\tau}_{d}$};

\draw[-stealth] (sum2.east) -- (M.west) node[midway,above](x){$\underline{F}_{c}$};
\draw[-stealth] (M.east) -- (sum3.west) node[midway,above](x){};
\draw[-stealth] (sum3.east) -- (Jt.west) node[midway,above](x){$F_{a}$};
\draw[-stealth] (Jt.east) -- (robot.west) node[midway,above](x){$\tau$};
...
\ExtractCoordinate{sum.west}
\draw[-stealth] (\XCoord-30, \YCoord) -- (\XCoord, \YCoord) node[midway,above]{$\underline{x}_{d}, \underline{\dot{x}}_{d}$};
\ExtractCoordinate{sum.south}
%(\XCoord, \YCoord-19) node[pos=0.15,above]{$\underline{x}$} -- 
%X feedback line
\draw[-stealth] (output.center) -- ++ (0,-4.5) -- (\XCoord, -4.5) -- (sum.south) node[below,left]{$-$};
...
% theta feedback line
\ExtractCoordinate{sum5.south}
\draw[-stealth] (output.center) -- ++ (0,-4.0) -- (\XCoord, -4.0) -- (sum5.south) node[below,left]{$-$};
\ExtractCoordinate{sum4.south}
\draw[-stealth] (output.center) -- ++ (0,-3.75) -- (\XCoord, -3.75) -- (sum4.south) node[below,left]{$-$};
\ExtractCoordinate{model.east}
\draw[-stealth] (output.center) -- ++ (0,\YCoord) -- (model.east);
...
\draw[-stealth] (sum5.north) -- (Kp.south);
\ExtractCoordinate{sum4.west}
\draw[-stealth] (-1.0, \YCoord) node[left]{$\underline{\dot{x}}_{d}$} -- (sum4.west) node[above]{$+$};
\ExtractCoordinate{sum5.west}
\draw[-stealth] (-1.0, \YCoord) node[left]{$\underline{x}_{d}$}-- (sum5.west) node[above]{$+$};
\end{tikzpicture}
\end{figure}
