...
\end{displaymath}
where $m_{i}$ is the mass of link $i$ and $\underline{\dot{v}}_{c_{i}}$ is the acceleration of link $i$'s CM relative to the base frame.

Euler's Equation is
\begin{displaymath}
	\underline{N}_{i} =\  ^{c_{i}}\!I\underline{\dot{\omega}}_{i} + \underline{\omega}_{i} \times x\ ^{c_{i}}\!I\underline{\omega}_{i}
\end{displaymath}
where $^{c_{i}}\!I$ is the inertia tensor of link $i$ relative to a frame $\{c_{i}\}$ whose origin is located at the link's CM, and $\underline{\omega}_{i}$ is the rotational velocity of link $i$. We get this by noting that
\begin{displaymath}
	\underline{N} = \frac{d}{dt} \left(\ ^{c}I\underline{\omega}\right)
\end{displaymath}
where $\underline{N}$ is the moment acting on joint $i$. We then expand the time--derivative of the right--hand side.

If you are not familiar with inertia tensors, they the rotational equivalent of the mass. But unlike mass, which is a scalar, inertia tensors are 3x3 matrices of the following form:
\begin{displaymath}
...
\begin{displaymath}
	^{A}I = \ ^{C}I + m\left[P^{T}_{c}P_{c}I_{3\times3} - P_{c}P_{c}^{T}\right]
\end{displaymath}
where $P_{c} = \left[x_{c}, y_{c}, z_{c}\right]^{T}$ locates the center of mass relative to frame $A$, and $I_{3\times3}$ is the 3x3 identity matrix.
...
\begin{displaymath}
	^{A}\underline{v}_{p} = \ ^{A}_{B}R\ ^{B}\underline{v}_{p} + \ ^{A}\underline{\Omega}_{B} \times \ ^{A}_{B}R\ ^{B}\underline{p}_{p} + \ ^{A}\underline{v}_{B_{O}}
\end{displaymath}

 where $^{A}\underline{v}_{B_{O}}$ is the velocity of the origin of frame $B$ with respect to frame $A$. Because linear accelerations are just the time derivatives of velocities, we can differentiate both sides of this equation to get what we need:
 \begin{eqnarray}
 	\frac{d}{dt}\left\{ \ ^{A}\underline{v}_{p}  \right\} & = &\left\{ \ ^{A}_{B}R \ ^{B}\underline{v}_{p} + \ ^{A}\underline{\Omega}_{B} \times \ ^{A}_{B}R\ ^{B}\underline{p}_{p} + \ ^{A}\underline{v}_{B_{O}} \right\} \nonumber \\
	^{A}\underline{\dot{v}}_{p} & = & \ ^{A}_{B}R\ ^{B}\underline{\dot{v}}_{p} + \ ^{A}_{B}\dot{R}\ ^{B}\underline{v}_{p} + \ ^{A}\underline{\dot{\Omega}}_{B}\times\ ^{A}_{B}R\ ^{B}\underline{p}_{p}+ \ ^{A}\underline{\Omega}_{B}\times \frac{d}{dt}\left(\ ^{A}_{B}R\ ^{B}\underline{p}_{p}\right) + \ ^{A}\underline{\dot{v}}_{B_{O)}} \nonumber \\
	&=& \ ^{A}_{B}R\ ^{B}\underline{\dot{v}}_{p} + \ ^{A}\underline{\Omega}_{B} \times\ ^{A}_{B}R\ ^{B}\underline{v}_{p} + \ ^{A}\underline{\dot{\Omega}}_{B}\times\ ^{A}_{B}R\ ^{B}\underline{p}_{p} \nonumber \\ && + \ ^{A}\underline{\Omega}_{B}\times(\ ^{A}\underline{\Omega}_{B}\times\ ^{A}_{B}R\ ^{B}\underline{p}_{p})  + \ ^{A}\underline{\Omega_{B}}\times\ ^{A}_{B}R\ ^{B}\underline{b}_{p} + \ ^{A}\underline{\dot{v}}_{B_{0}} \nonumber
\end{eqnarray}
Simplifying, we end up with
\begin{displaymath}
	^{A}\underline{\dot{v}}_{p} = \ ^{A}\underline{\dot{v}}_{B_{0}} + \ ^{A}_{B}R\ ^{B}\underline{\dot{v}}_{p} + 2\ ^{A}\underline{\Omega}_{B}\times\ ^{A}_{B}R\ ^{B}\underline{v}_{p} + \ ^{A}\underline{\dot{\Omega}}_{B}\times\ ^{A}_{B}R\ ^{B}\underline{p}_{p} + \ ^{A}\underline{\Omega}_{B}\times\left(\ ^{A}\underline{\Omega}_{B}\times\ ^{A}_{B}R\ ^{B}\underline{p}_{P}\right)
\end{displaymath}
...
We'll now use these results to do a forward sweep from the base frame to the tooltip, calculating accelerations as we go. First we need an expression for the linear acceleration of the link frame origin:
\begin{eqnarray}
...
&& + 2\ ^{i+1}\underline{\omega}_{i+1}\times\dot{d}_{i+1}\ ^{i+1}\hat{z}_{i+1} + \dot{d}_{i+1}\ ^{i+1}\hat{z}_{i+1}
\end{eqnarray}
Next, an expression for the linear acceleration of the link Center of Mass:
\begin{eqnarray}
^{i}_{0}R\left\{\ ^{0}\underline{\dot{v}}_{c_{i}} \right. & = & \left. \ ^{0}_{i}R\ ^{i}\underline{\dot{v}}_{c_{i}} + \ ^{0}\underline{\dot{\Omega}}_{i}\times\ ^{0}_{i}R\ ^{i}\underline{p}_{c_{i}}+\ ^{o}\underline{\dot{v}}_{i} + 2\ ^{0}\underline{\Omega}_{i}\times\ ^{0}_{i}R\ ^{i}\underline{v}_{c_{i}} + \ ^{0}\underline{\Omega}_{i}\times (\ ^{0}\underline{\Omega}_{i}\times\ ^{0}_{i}R\  ^{i}\underline{p}_{c_{i}}) \right\} \nonumber \\
...
Now we'll look at the angular acceleration of link frames:
\begin{eqnarray}
...
^{i+1}_{i}R\left\{\ ^{0}\underline{\dot{\omega}}_{i+1}\right\} & = & \ ^{i+1}_{i}R\left\{\ ^{i}\underline{\dot{\omega}}_{i} + \ ^{i}_{i+1}R\ ^{i+1}\hat{z}_{i+1}\ddot{\theta}_{i+1} + \ ^{i}\underline{\Omega}_{i+1}\!\times\ \!^{i}_{i+1}R\ ^{i+1}\hat{z}_{i+1}\dot{\theta}_{i+1} \right. \nonumber \\
...
\end{equation}
We've already derived an expression for propagate the angular velocity:
\begin{equation}
...
\begin{eqnarray}
	^{f}\underline{f}_{i} & = & ^{i}_{i+1}R\ ^{i+1}\underline{f}_{i+1} + \ ^{i}\underline{F}_{i} \\
	^{i}\underline{n}_{i} & = & \ ^{i}\underline{N}_{i} + \ ^{i}_{i+1}R\ ^{i+1}\underline{n}_{i+1} + \ ^{i}\underline{p}_{c_{i}}\times\ ^{i}\underline{f}_{i}+\ ^{i}\underline{p}_{i+1}\times\ ^{i}_{i+1}R\ ^{i+1}\underline{f}_{i+1}
\end{eqnarray}
...
\end{equation}
For an articulated rigid body manipulator,
\begin{eqnarray}
...
Note that we can calculate $^{i}\underline{v}_{c_{i}}$ as follows:
\begin{eqnarray}
^{0}\underline{v}_{c_{i}} & = & ^{0}\underline{\Omega}_{i}\times\ ^{0}_{i}R\ ^{i}\underline{p}_{c_{i}} + \ ^{0}\underline{v}_{i} \nonumber \\
^{i}_{0}R\left\{ \underline{v}_{c_{i}} \right. & = & \left. \underline{\omega}_{i}\times\ ^{0}_{i}R\ ^{i}\underline{p}_{0}+\underline{v}_{i} \right\} \nonumber \\
^{i}\underline{v}_{c_{i+1}} & = & \ ^{i+1}R_{i}\left(\ ^{i}\underline{v}_{i} + \ ^{i}\underline{\omega}_{i}\times\ ^{i}\underline{p}_{i+1}\right) + \underline{\dot{d}}_{i+1}\ ^{i+1}\hat{z}_{i+1} \nonumber
\end{eqnarray}
...
Also,
\begin{equation}
P(\underline{q}) = \sum_{i=1}^{N} -m_{i}\ ^{0}\underline{g}^{T}\ ^{0}\underline{p}_{c_{i}} = E(\underline{q})\nonumber
\end{equation}
which nearly always refers only to gravitational terms.
...
\begin{equation}
	\underline{\tau} = \frac{1}{2}\underline{\dot{q}}^{T}M(\underline{q})\underline{\dot{q}} + b(\underline{q}, \underline{\dot{q}}) + E(\underline{q})
\end{equation}
This is known as a ``Hamiltonian system'', and is quite general. $E(\cdot) = \frac{\partial P}{\partial \underline{q}}$ is usually just gravitational forces, but can also include frictional terms or other terms exerted on the manipulator by the environment.

In fact, we can generally further factor the equations of motion as
\begin{equation}
	\underline{\tau} = \frac{1}{2}\underline{\dot{q}}^{T}M(\underline{q})\underline{\dot{q}} + B(\underline{q})\underline{\dot{q}} + C(\underline{q})\underline{\dot{q}}^{2} + E(\underline{q})
\end{equation}
...
\begin{eqnarray}
	...
	&&+(m_{1}+m_{2})l_{2}^{2}\ddot{\theta}_{1}-m_{2}l_{1}l_{2}s_{2}\dot{\theta}_{2}^{2} - 2m_{2}l_{1}l_{2}s_{2}\dot{\theta}_{1}\dot{\theta}_{2} + m_{2}l_{2}gc_{12}+(m_{1}m_{2})l_{1}gc_{1} \nonumber \\
...
\begin{equation}
\underline{\tau} = M(\underline{q})\ddot{q} + B(\underline{q})\underline{\dot{q}} + C(\underline{q})\underline{\dot{q}}^{2}+G(\underline{q}) \nonumber
\end{equation}
where
\begin{displaymath}
