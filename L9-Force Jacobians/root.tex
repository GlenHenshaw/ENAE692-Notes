...
\underline{n}_{i} & \text{torque exerted on link i by link i-1} \nonumber
\end{eqnarray}
...
\begin{displaymath}
	\underbrace{\mathcal{F} \cdot \delta\mathcal{X}}_{\substack{\text{Cartesian}\\\text{space}}} = \underbrace{\tau \cdot \delta\\underline(q)}_{\substack{\text{Joint}\\ \text{space}}}
\end{displaymath}
where $\mathcal{F}$ is a Cartesian force--moment vector acting on the end effector, $\delta\mathcal{X}$ is a infinitesimal Cartesian displacement of the end effector, $\tau$ is the vector of torques (or forces, for prismatic actuators) at the joints, and $\delta \underline{q}$ is an infinitesimal vector of joint displacements.

You can write a dot product as a vector multiplication, so
\begin{displaymath}
\underbrace{\mathcal{F}^{T}\delta\mathcal{X}}_{\substack{\text{Cartesian}\\\text{space}}} = \underbrace{\tau^{T} \delta\underline{q}}_{\substack{\text{Joint}\\ \text{space}}}
\end{displaymath}
...
\begin{displaymath}
\mathcal{F}^{T}J\delta\Theta = \tau^{T}\delta\underline{q}
\end{displaymath}
And this has to be true for \textbf{any} value of $\delta \underline{q}$, so we can cancel this term out to get
\begin{displaymath}
\mathcal{F}^{T}J = \tau^{T}
\end{displaymath}
or
...
\end{displaymath}
 Be careful not to confuse $T_{v}$ with a generalized coordinate transformation matrix, which is often denoted as $^{i+1}_{i}T\ $!

In exercise 5.1 you are asked to derive the expression for $T_{v}^{-1}$, which is
\begin{displaymath}
\left[\begin{array}{c}^{A}v_{A} \\ ^{A}\omega_{A}\end{array}\right] = \left[\begin{array}{cc}^{A}_{B}R & ^{A}P_{B,Org}\!\times\ \!^{A}_{B}R\ \\ 0 & ^{A}_{B}R\end{array}\right]\left[\begin{array}{c}^{B}v_{B} \\ ^{B}\omega_{B}\end{array}\right]
\end{displaymath}
