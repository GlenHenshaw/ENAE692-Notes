...
\begin{displaymath}
 ...
 or
 \begin{displaymath}
 M(\underline{q})(\ddot{\underline{q}} - \ddot{\underline{q}}_{d} + K_{p}(\underline{q} - \underline{q}_{d})+ K_{v}(\underline{\dot{q}} - \underline{\dot{q}}_{d})))  = 0 
\end{displaymath}
and if we define
\begin{eqnarray}
...
\begin{displaymath}
 ...
 \textbf{It is very important to note} that this equation we have derived is in fact a differential equation that describes the dynamics of the SERVO ERROR. This is not the dynamics of the actual robot; this is the equation that describes how the tracking error of the robot evolves over time. This is a game we often play in nonlinear controls: we try to come up with a control law that gives us a differential equation describing the evolution of the tracking error, and then try to craft a control law that makes the state of this differential equation converge to zero. This implies that the control law will (asymptotically) perfectly track a desired trajectory, even if that trajectory is varying with time.
...
\end{displaymath}
where $\hat{M}$ is our best guess at the true $M$ matrix, and so on. If you plug this controller into Equation \ref{hamiltonian}, you end up with the following servo error dynamics:
\begin{displaymath}
...
Lyapunov generalized this insight slightly. Lyapunov theory posits that if you can write down \textbf{any} function of the state of the system which is always positive except at the origin, and if the time derivative of this equation evaluated along the trajectories of the system is always ``non--positive'' except at the origin (also known as \textit{negative semi--definite)}, then the state has to asymptotically converge to zero. Such a function is called a Lyapunov function. If you think of a Lyapunov function as the energy of the system, or a sort of pseudo--energy of the system, then you'll have the right intuition.
...
\begin{equation}
 \mathrm{V} = \frac{1}{2}m\dot{x}^{2} + \frac{1}{2}kx^{2} \label{smdenergy}
\end{equation}
and the derivative of this is
\begin{displaymath}
 \dot{\mathrm{V}} = m\dot{x}\ddot{x}+kx\dot{x}
\end{displaymath}
we can substitute for $m\ddot{x}$ from Equation \ref{smd2} to get
\begin{equation}
 \dot{\mathrm{V}} = -b\dot{x}^{2} \label{smdenergyderiv}
\end{equation}
...
OK, let's scale up a bit and see if we can use this approach for the manipulator described by Equation \ref{hamiltonian} and a simplified computed--torque controller where we assume we know exactly the Coriolis and centripetal forces and the environmental forces but not the mass matrix. And in order to make the math easier, we'll also assume that our mass estimate $\hat{M}$ is constant (although the actual mass matrix $M(\underline{q})$ isn't). This is basically a linear PD control law with an extra term that tries to cancel out environmental forces like gravity. This is an extremely common control design in industrial manipulators.
...
%Substituting Equation \ref{simplect} into Equation \ref{hamiltonian}, we get
%\begin{eqnarray}
% M\ddot{\underline{q}} + C\dot{\underline{q}} + E & = & \hat{M}\left(-K_{p}\tilde{\underline{q}} - K_{v}\dot{\tilde{\underline{q}}}\right) + C\dot{\underline{q}} + E \nonumber \\
% M\underline{\ddot{q}} + \hat{M}K_{p}\tilde{\underline{q}} + \hat{M}K_{v}\dot{\tilde{\underline{q}}} & = & 0 \nonumber \\
%\end{eqnarray}
%Because of our godlike intelligence, we're going to pull a candidate Lyapunov function out of the air:
%\begin{displaymath}
% V = \frac{1}{2}\dot{\tilde{\underline{q}}}^{T}M\underline{\dot{\tilde{q}}} + \frac{1}{2}\underline{\tilde{q}}^{T}\hat{M}K_{p}\underline{\tilde{q}}
%\end{displaymath}
%which is a positive definite function of $\tilde{\underline{q}}, \dot{\tilde{\underline{q}}}$.
