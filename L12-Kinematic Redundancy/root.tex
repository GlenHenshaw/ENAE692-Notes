...
\end{displaymath}
where $\gamma$ is a gain. In this case, it lets you ``trade off'' how much you care about small joint velocities versus how much you care about staying away from collisions. We proceed as above:
\begin{displaymath}
...
\begin{eqnarray}
    ...
    & = & J^{\dag} \Delta\underline{x} + \gamma \left[J^{\dag}J - I\right] \frac{\partial D}{\partial \Delta\underline{q}}^{T}\Delta t 
\end{eqnarray}
...
Editor's note: here is the original diagram taken from the paper that details this analysis, which may be more helpful (Kreutz-Delgado, Kenneth et al. “Kinematic analysis of 7 DOF anthropomorphic arms.” Proceedings., IEEE International Conference on Robotics and Automation (1990): 824-830 vol.2.):
\begin{center}
\includegraphics[height=3in]{armdiagram.png}
\end{center}

Define 
\begin{eqnarray}
 \underline{w} & = & \overrightarrow{sw} = \underline{p}_{w}-\underline{p}_{s} \nonumber \\
...
\end{displaymath}
and note that the last three entries do not depend on $\theta_{5}, \theta_{6}, \theta_{7}$. So the extended Jacobian is:
\begin{equation}
...
\begin{displaymath}
\Delta\underline{q} = J^{\dag}\Delta\underline{x} + \gamma \left[ J^{\dag}J-I\right] \frac{\partial D}{\partial \Delta\underline{q}}^{T}\Delta t
\end{displaymath}
...
\begin{displaymath}
	\frac{\partial D}{\partial \Delta\underline{q}} = 2\left[
		\begin{matrix}
			\frac{q_{0} - q_{0}^{\mathrm{mid}}}{q_{0}^{\mathrm{min}} - q_{0}^{\mathrm{max}}} & 0 & \dots & 0 \\
...
\begin{displaymath}
\Delta\underline{q} = J^{\dag}\Delta\underline{x} + \gamma \left[ J^{\dag}J-I\right]G\ \Delta t
\end{displaymath}
