...
\begin{equation}
 \tau_{i} = m_{i}\ddot{q}_{i} + b_{i}\dot{q}_{i}+e_{i}(q_{i}) \label{SMD}
\end{equation}
...
This is the step response for an underdamped system:
\begin{figure}[h!]
 \centering
 \begin{tikzpicture}
    \begin{axis}[
...
where:
\begin{eqnarray}
    & M_{p} = e^{\frac{-\zeta\omega_{n}\pi}{\omega_{d}}} \times 100\% & \ \ \ \text{``Peak Overshoot''} \nonumber\\
